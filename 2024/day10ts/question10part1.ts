...
const incrementingPathEndpoints = new Map<Cell<number>, Set<Cell<number>>>();

const countIncrementingPaths = (grid:Grid<number>, cell:Cell<number>, currentPath:Cell<number>[]):number => {
    if (cell.val === 9) {
        if (!incrementingPathEndpoints.get(currentPath[0])) {
            incrementingPathEndpoints.set(currentPath[0], new Set())
        }
        const newSet = incrementingPathEndpoints.get(currentPath[0])!;
        newSet?.add(currentPath[currentPath.length-1]);
        incrementingPathEndpoints.set(currentPath[0], newSet)
...
for (let [k,v] of incrementingPathEndpoints.entries()) {
    scores.set(`(${k.x},${k.y})`, (scores.get(`(${k.x},${k.y})`) || 0) + v.size)
}
