...
lines.forEach((l,id) => {
    ...
    if (vector.startsWith('U')) {
        console.log(`Moving Up ${magnitude} from ${current.x}, ${current.y} (${wireSteps})`)
        for (let i = 1; i <= magnitude; i++) {
            ...
            if (destination.val !== '.' && destination.val !== '*' && destination.val !== `${id}`) {
                destination.val = '*'
                destination.wireSteps += wireSteps
                crossovers.push(destination)
                console.log(`Adding: ${destination.x},${destination.y}, ${destination.wireSteps}`)
    ...
    if (vector.startsWith('D')) {
        console.log(`Moving Down ${magnitude} from ${current.x}, ${current.y} (${wireSteps})`)
        for (let i = 1; i <= magnitude; i++) {
            wireSteps += 1
            destination = grid.rows[current.y + i].cells[current.x]
            if (destination.val !== '.' && destination.val !== '*' && destination.val !== `${id}`) {
    ...
    if (vector.startsWith('L')) {
        console.log(`Moving Left ${magnitude} from ${current.x}, ${current.y} (${wireSteps})`)
        for (let i = 1; i <= magnitude; i++) {
            wireSteps += 1
    ...
    if (vector.startsWith('R')) {
        console.log(`Moving Right ${magnitude} from ${current.x}, ${current.y} (${wireSteps})`)
        for (let i = 1; i <= magnitude; i++) {
            wireSteps += 1
            destination = grid.rows[current.y].cells[current.x + i]
            if (destination.val !== '.' && destination.val !== '*' && destination.val !== `${id}`) {
...
console.log(crossovers.map(c => {
    return ({manhattan: Math.abs(c.x - start.x) + Math.abs(c.y - start.y), x: c.x, y: c.y, wireSteps: c.wireSteps })
}).sort((m1,m2) => m1.wireSteps - m2.wireSteps)[0])